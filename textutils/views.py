...
def analyze(request):
    text={"given":(request.GET.get("Text","")),"analyzed":(request.GET.get("Text","")),"operations":[]}
    if text=="":
        return render(request,"analyze.html",text, status=400)
    
    remove_punc=(request.GET.get("removepunc",False))
    upper_case=(request.GET.get("upper",False))
    lower_case=(request.GET.get("lower",False))
    remove_newline=(request.GET.get("rm_newline",False))
    ext_space_rm=(request.GET.get("rm_ext_space",False))
